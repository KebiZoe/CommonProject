...
//            _currentViewOrViewControllor = view;
            [self setValue:view forKey:@"currentViewOrViewControllor"];
        }
    }];
    _array = arrayM;
}
...
- (void)setupScrollViewWithViewControllers:(NSArray *)viewControllers type:(SegmentViewControlIsLazyLoadType)type recognizerTableCellEdit:(BOOL)rgCellEdit
{
    ...
    [viewControllers enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx==0) {
            ...
            [self setValue:controller forKey:@"currentViewOrViewControllor"];
...
//    _currentViewOrViewControllor = _array[index];
    [self setValue:_array[index] forKey:@"currentViewOrViewControllor"];
    // 滚动scrollView
...
//            }];
    ...
    if (self.item.margin>=0) {
        [self.titlesView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.mas_equalTo(lastButton.right).offset(self.item.margin);
        }];
    }else{
        [self.titlesView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.mas_equalTo(lastButton.right).offset(margin);
        }];
    }
    
    
    if (self.item.lineIndicator_percent>0&&self.item.lineIndicator_percent<1) {
        [self.titleUnderline mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.bottom.mas_equalTo(self.titlesView.bottom);
            make.height.offset(2);
            make.centerX.equalTo(self.previousClickedTitleButton);
            make.width.mas_equalTo(self.previousClickedTitleButton.width).multipliedBy(self.item.lineIndicator_percent);
        }];
    }else{
        [self.titleUnderline mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.bottom.mas_equalTo(self.titlesView.bottom);
        ...
        if (_type == SegmentViewControlIsLazyLoad) {
            ...
            [_array enumerateObjectsUsingBlock:^(__kindof UIViewController*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if (idx == 0) {
                    ...
                    [obj.view mas_makeConstraints:^(MASConstraintMaker *make) {
                        make.left.mas_equalTo(lastView.right);
                        make.top.bottom.equalTo(self.contentView);
                        make.width.equalTo(self.scrollView);
                    }];
                    lastView = obj.view;
